...
def draw(img, bboxes, kpss, out_path, with_kps=True):
    ...
    for i in range(bboxes.shape[0]):
        ...
        if cls_id == 3:
            ...
            if qr_roi.size > 0:
                # 保存裁剪区域为单独整图，再从磁盘读取该整图用于解码（确保使用原始像素）
                base_dir = os.path.dirname(out_path) or "."
                crops_dir = os.path.join(base_dir, "crops")
                os.makedirs(crops_dir, exist_ok=True)
                base_name = os.path.splitext(os.path.basename(out_path))[0]
                crop_path = os.path.join(crops_dir, f"{base_name}_crop_{i}.png")
                cv2.imwrite(crop_path, qr_roi)
                # 从磁盘读取刚保存的裁剪图（确保后续操作基于保存的原始像素）
                crop_bgr = cv2.imread(crop_path)
                if crop_bgr is None:
                    # 读取失败则退回使用内存中的裁剪数据
                    crop_bgr = qr_roi.copy()
                # 转为灰度 uint8（与 sr.run_sr_on_gray 的输入约定一致）
                if crop_bgr.ndim == 3:
                    gray_roi = cv2.cvtColor(crop_bgr, cv2.COLOR_BGR2GRAY)
                else:
                    gray_roi = crop_bgr.copy()
                # 先尝试直接解码（与 sr.decode_qr 行为一致）
                try:
                    res_direct = decode_qr(gray_roi)
                except Exception:
                    # 万一 decode_qr 抛异常，降级为 OpenCV 单码接口
                    det = cv2.QRCodeDetector()
                    txt, pts, _ = det.detectAndDecode(gray_roi)
                    res_direct = [(txt.strip(), pts)] if txt else []
                if res_direct:
                    for txt, _ in res_direct:
                        print(f"[QR] 裁剪并保存后直接解码到二维码内容: {txt}  (crop saved: {crop_path})")
                else:
                    # 直接解码失败，若 SR 引擎可用则对保存的裁剪图做灰度 SR 再解码
                    if sr_net is not None:
                        try:
                            ...
                            if res_sr:
                                for txt, _ in res_sr:
                                    print(f"[QR] SR 后（基于保存裁剪图）解码到二维码内容: {txt}  (crop saved: {crop_path})")
                            else:
                                print(f"[QR] 裁剪图直接灰度和 SR 后均未能解码二维码 (crop saved: {crop_path})")
                        except Exception as e:
                            print(f"[QR] SR 处理或解码时出错: {e} (crop saved: {crop_path})")
                    else:
                        print(f"[QR] 裁剪图直接灰度解码失败，且未加载 SR 引擎，无法继续 SR 解码 (crop saved: {crop_path})")
